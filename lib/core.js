...
exports.testmail = function(req,res,next){
  ...
  mail.sendMail(message, user, res);
};
...
exports.createUser = function(req, res , next) {
    ...
    mail.sendMail(mailmessage, user, res);
...
exports.createApplication = function(req, res , next) { 
    ...
    mailmessage = {
      ...
      content: "Hi "+user.name+", \n you have a new application to the body, by "+user.applicant
     };
...
exports.modifyMembership = function(req, res , next) {
    ...
    ldap.modifyMembership(baseDN, req.params.memberType, function(filter, searchDN, res) {
      ...
      ldap.searchLDAP(filter,searchDN,res, function(res, data){
                  ...
                  //send mail accordingly
                  var mailmessage = {
                     to:      user.fullName+" <"+user.mail+">",
                     subject: "Your membership has changed",
                     content:    "Hi "+user.name+", \n your membership has changed to "+user.memberType
                  };
                  mail.sendMail(mailmessage, user, res);
    ...
    }(filter,searchDN,res) ); //FIXME: The function is called twice, probably: 
                              //once after declared and once invoked
                              //probably fix with .bind(<params>) as per http://stackoverflow.com/questions/11371673/pass-additional-parameter-to-javascript-callback-function/28120741#28120741
