...
exports.authenticate = function(req, res, next) {
  ...
  client.log.info(user, 'User is requesting a token');
      
  // find the user  
  client.bind('uid='+uid+',ou=people,o=aegee,c=eu', password, function(err) {
    if(err){
...
exports.modifyMembership = function(req, res , next) {
    ...
    //_ldap.modifyMembership(baseDN, req.params.memberType, function(filter, searchDN, res,xxx){searchLDAP(filter,searchDN,res,xxx)})
...
//BUG: modify is async and it may not finish before searchLDAP
    var searchDN = 'uid=' + req.params.userId + ',ou=people, ' + ldap_top_dn;
    var filter = '(&(bodyCode=' + req.params.bodyCode + ')(objectClass=aegeePersonMembership))';

     searchLDAP(filter, searchDN, res, function(res, data){
...
//v0.1.2
function searchLDAP(searchFilter, searchDN, res, next) {
